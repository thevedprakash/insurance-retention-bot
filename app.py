...
from bot.leonardo_agent import send_summary_to_supervisor  # Import Leonardo's functionality
...
def user_response():
    try:
        ...
        if any(phrase in user_message.lower() for phrase in end_conversation_phrases):
            ...
            logging.info("send_summary_to_supervisor.")
            send_summary_to_supervisor()

            # Move to the next customer
            logging.info("next_customer.")
            response = next_customer()
            return response

        # Continue the conversation if not ending
        rag_gpt_agent.human_step(user_message)
        bot_message = rag_gpt_agent.step().replace('<END_OF_TURN>', '').strip()
        ...
        logging.error(f"Error during user response processing: {e}")
        return jsonify({"error": str(e)}), 500
...
def next_customer():
    try:
        ...
        initial_bot_message = rag_gpt_agent.step().replace('<END_OF_TURN>', '').strip()
        ...
        logging.error(f"Error during switching to next customer: {e}")
        return jsonify({"error": str(e)}), 500
